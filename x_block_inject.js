/**
 * @name X_Web_Block_Injector
 * @desc 移除 X.com 网页 CSP 限制，并注入 1-Click Block 油猴脚本
 */

let body = $response.body;
let headers = $response.headers;

// 1. 移除 CSP (Content-Security-Policy)，防止 X 拦截注入的执行
let newHeaders = {};
...
// 2. 将油猴脚本逻辑放入此函数内部（利用函数封装避免引号转义冲突）
function injectedCode() {
    console.log("[Loon] X 1-Click Block 脚本开始加载...");

    // =========================================================
    // ▼▼▼ 请将 GreasyFork 脚本的核心逻辑直接粘贴在下方 ▼▼▼
    // (注意：只需剔除开头的 ==UserScript== 块即可，原代码原样粘贴)
    // 
    // 若原脚本使用了 GM_addStyle，请替换为标准写法，例如：
    // const style = document.createElement('style');
    // style.textContent = `此处放CSS内容`;
...
// 3. 将函数转为字符串，并拼装成 HTML <script> 标签
const scriptStr = '<script>try { (' + injectedCode.toString() + ')(); } catch(e) { console.error("[Loon Inject Error]", e); }</script>';

// 4. 执行 DOM 注入：找到 </body> 标签并在其前方插入
if (body && typeof body === "string") {
    if (body.indexOf("</body>") !== -1) {
        body = body.replace("</body>", scriptStr + "</body>");
    } else {
        // 如果 HTML 结构不完整，直接追加到末尾
        body += scriptStr;
    }
}

// 5. 返回修改后的 Headers 和 Body 给 Loon 引擎
